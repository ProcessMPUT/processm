...
object UserGroupWithWorkspaces : IdTable<UUID>("user_groups_with_workspaces") {
    ...
    override val primaryKey = PrimaryKey(userGroupId, workspaceId, organizationId)
    override val id: Column<EntityID<UUID>>
        get() = userGroupId
}

class UserGroupWithWorkspace(userGroupId: EntityID<UUID>) : Entity<UUID>(userGroupId) {
    companion object : EntityClass<UUID, UserGroupWithWorkspace>(UserGroupWithWorkspaces)
    ...
    fun toDto() = UserGroupWithWorkspaceDto(userGroup.toDto(), workspace.toDto(), organization.toDto())
}
