...
export class PetriNetState {
  ...
  clone(): PetriNetState {
    ...
    [...this._places.values()].forEach((place) =>
      newState.createPlace(place.getOptions())
    );
    [...this._transitions.values()].forEach((transition) =>
      newState.createTransition(transition.getOptions())
    );
    [...this._arcs.values()].forEach((arc) =>
      newState.createArc(arc.outElementId, arc.inElementId, arc.id)
    );
  ...
  createArc(outId: string, inId: string, id?: string): Arc | null {
    ...
    const arcIndex = this._elementArcs
      .get(outElement.id)!
  ...
  removeArc(arcId: string): void {
    ...
    this._elementArcs.set(
      arc.inElementId,
      this._elementArcs.get(arc.inElementId)!.filter((arc1) => arc1.id != arcId)
    );
    this._elementArcs.set(
      arc.outElementId,
      this._elementArcs
        .get(arc.outElementId)!
        .filter((arc1) => arc1.id != arcId)
    );
  ...
  getPrePrePlaces(place: Place): Place[] {
    return this.getPreTransitions(place).flatMap((transition) =>
      this.getPrePlaces(transition)
    );
  }

  getPostPostPlaces(place: Place): Place[] {
    return this.getPostTransitions(place).flatMap((transition) =>
      this.getPostPlaces(transition)
    );
  }
  ...
  isCorrectNet(): [boolean, string] {
    const incorrectTransitions = this.transitions.filter((transition) => {
      return (
        this.getPrePlaces(transition).length == 0 ||
        this.getPostPlaces(transition).length == 0
      );
    });

    let message: string | null = null;
    if (incorrectTransitions.length != 0) {
      const transitionTexts = incorrectTransitions
        .map((transition) => transition.text)
        .join(", ");
      message = `Incorrect transitions found: ${transitionTexts}`;
    }

    const incorrectPlaces = this.places.filter((place) => {
      return (
        this.getPreTransitions(place).length == 0 &&
        this.getPostTransitions(place).length == 0
      );
    });
