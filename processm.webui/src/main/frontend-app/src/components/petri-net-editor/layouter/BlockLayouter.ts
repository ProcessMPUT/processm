...
import { AbstractionReductionFinder } from "@/components/petri-net-editor/layouter/finders/AbstractionReductionFinder";
...
import { TransitionReductionFinder } from "@/components/petri-net-editor/layouter/finders/TransitionReductionFinder";
...
export class BlockLayouter implements Layouter {
  ...
  private static readonly MAX_SCORE: number = Math.max(
    ...BlockLayouter.REDUCTION_RULES.map((finder) => finder.getMaxScore())
  );
  ...
  private updateStatePositions(
    state: PetriNetState,
    resultState: PetriNetState
  ) {
