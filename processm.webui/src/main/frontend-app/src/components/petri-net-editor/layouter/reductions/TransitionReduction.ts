...
export class TransitionReduction implements Reduction {
  ...
  private static isBetter(block: Block, bestWidth: number, bestHeight: number) {
    return (
      block.height > bestHeight ||
      (block.height == bestHeight && block.width > bestWidth)
    );
  }
  ...
  apply(state: PetriNetState): Block {
    ...
    if (this._transitions.length == 0) {
      const place: Place = prePlaces.length == 0 ? postPlaces[0] : prePlaces[0];
      ...
      const diff: PetriNetElement[] = [...prePlaces].filter((place) =>
        postPlaces.has(place)
      );
      ...
      const transitionsIds = new Set<string>(
        this._transitions.map((transition) => transition.id)
      );
      ...
      if (state.getPrePlaces(this._transitionToReduce).length != 0) {
        ...
        neighborTransition = state
          .getPostTransitions(prePlace)
  ...
  getScore(state: PetriNetState): number {
    ...
    const prePlacesAcceptable = state
      .getPrePlaces(this._transitionToReduce)
    ...
    const postPlacesAcceptable = state
      .getPostPlaces(this._transitionToReduce)
