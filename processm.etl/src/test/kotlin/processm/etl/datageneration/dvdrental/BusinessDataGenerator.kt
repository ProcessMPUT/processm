...
class BusinessDataGenerator {
    ...
    private val executedActivitiesCount = businessProcessExecutionFrequencies.keys.map { it to 0 }.toMap().toMutableMap()
    ...
    @ExperimentalTime
    private fun getRandomTimestampInThePast(after: Long? = null, before: Long? = null): DateTime  {
        val atLeast = before ?: Duration.convert(1.0, sourceUnit = DurationUnit.DAYS, targetUnit = DurationUnit.MILLISECONDS).toLong()
        val atMost = after ?: Duration.convert(365.0, sourceUnit = DurationUnit.DAYS, targetUnit = DurationUnit.MILLISECONDS).toLong()
        return getCurrentTime() - Random.nextLong(atLeast, atMost)
    }

    @ExperimentalTime
    private fun getRandomTimestampInTheFuture(after: Long? = null, before: Long? = null): DateTime  {
        val atLeast = before ?: Duration.convert(1.0, sourceUnit = DurationUnit.DAYS, targetUnit = DurationUnit.MILLISECONDS).toLong()
        val atMost = after ?: Duration.convert(365.0, sourceUnit = DurationUnit.DAYS, targetUnit = DurationUnit.MILLISECONDS).toLong()
        return getCurrentTime() + Random.nextLong(atLeast, atMost)
    }
