...
class CausalNetTraceComparison(val left: Model, val right: Model) {
    ...
    val leftTraces by lazy {
        leftVerificationReport.validLoopFreeSequences.map { seq -> seq.map { it.a }.filterNot { it.special } }.toSet()
    }
    ...
    val rightTraces by lazy {
        rightVerficiationReport.validLoopFreeSequences.map { seq -> seq.map { it.a }.filterNot { it.special } }.toSet()
    }
