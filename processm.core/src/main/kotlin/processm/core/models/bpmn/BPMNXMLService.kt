...
import processm.core.models.bpmn.BPMNXMLService.validate
 ...
 * This does not include the function to validate the XML ([validate]), as its interface relies only on standard classes.
 */
object BPMNXMLService {
    ...
    private val validator =
        SchemaFactory.newInstance(XMLConstants.W3C_XML_SCHEMA_NS_URI)
            .newSchema(javaClass.classLoader.getResource("bpmn/BPMN20.xsd")).newValidator()

    private val logger = logger()

    /**
     * @return `true` if [xml] conforms to the BPMN XSD, `false` otherwise
     */
    fun validate(xml: InputStream): Boolean {
        return try {
            ...
            logger.debug("BPMN XML validation failed", e)
            false
        }
    }
}
