...
class ActivityBindingTest {
    ...
    fun `state PM page 75`() {
        ...
        val d = Node("d")
        val e = Node("e")
        val s1 = ActivityBinding(a, setOf(), setOf(b, d), setOf()).state
        assertEquals(setOf(a to b, a to d), s1)
        val s2 = ActivityBinding(d, setOf(a), setOf(e), s1).state
        assertEquals(setOf(a to b, d to e), s2)
        val s3 = ActivityBinding(b, setOf(a), setOf(e), s2).state
        assertEquals(setOf(b to e, d to e), s3)
