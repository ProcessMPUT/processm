...
class ActivityBindingTest {
    ...
    fun `state PM page 75`() {
        ...
        assertEquals(setOf(Dependency(a, b), Dependency(a, d)), s1.uniqueSet().asCollection())
        val s2 = ActivityBinding(d, setOf(a), setOf(e), s1).state
        assertEquals(setOf(Dependency(a, b), Dependency(d, e)), s2.uniqueSet().asCollection())
        val s3 = ActivityBinding(b, setOf(a), setOf(e), s2).state
        assertEquals(setOf(Dependency(b, e), Dependency(d, e)), s3.uniqueSet().asCollection())
