...
export class PetriNetState {
    ...
    getLayoutMap(): Map<string, Block> {
        return new Map<string, Block>(this._layoutMap);
    }

    setLayoutMap(layoutMap: Map<string, Block>) {
        this._layoutMap = new Map<string, Block>(layoutMap);
    }

    resetLayoutMap(): void {
        this._layoutMap.clear();
        for (const element of [...this.getPlaces(), ...this.getTransitions()]) {
    ...
    isCorrectNet(): [boolean, string] {
        const incorrectTransitions = this.getTransitions()
            .filter(transition => {
        ...
        const incorrectPlaces = this.getPlaces()
            .filter(place => {
