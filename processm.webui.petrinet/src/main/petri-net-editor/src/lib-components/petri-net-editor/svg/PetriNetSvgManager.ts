import { Place, PlaceOptions } from '@/lib-components/petri-net-editor/model/Place';
import { Transition, TransitionOptions } from '@/lib-components/petri-net-editor/model/Transition';
import type { EventBus, SVGLineSelection, SVGSelection } from '@/utils/Types';
import type { PetriNetElement } from '@/lib-components/petri-net-editor/model/PetriNetElement';
import { EventNames } from '@/utils/Constants';
import * as d3 from 'd3-selection';
import { PetriNetState } from '@/lib-components/petri-net-editor/model/PetriNetState';
import { SvgArc } from '@/lib-components/petri-net-editor/svg/SvgArc';
...
import { NodeBlock } from '@/lib-components/petri-net-editor/layouter/blocks/NodeBlock';
...
export class PetriNetSvgManager {
    ...
    private svgTransitions: Map<string, SvgTransition> = new Map<string, SvgTransition>();
    ...
    constructor(svg: SVGSelection) {
        this.svg = svg;
        this.state = new PetriNetState();
        this.enableDragging();
    }

    getState(): PetriNetState {
        return this.state;
    }

    setState(newState: PetriNetState): void {
        this.state.getArcs()
            .forEach(arc => this.removeArc(arc.id));

        [...this.state.getPlaces(), ...this.state.getTransitions()]
            .forEach(element => this.removeElement(element.id));

        this.state = new PetriNetState();
        newState.getPlaces().forEach(place => this.createPlace(place.getOptions()));
        newState.getTransitions().forEach(transition => this.createTransition(transition.getOptions()));
        newState.getArcs().forEach(arc => this.connect(arc.outElementId, arc.inElementId));
        this.state.setLayoutMap(newState.getLayoutMap());
    }

    createPlace(options: PlaceOptions): string {
        const place = this.state.createPlace(options);
    ...
    createTransition(options: TransitionOptions): string {
        const transition = this.state.createTransition(options);
        this.svgTransitions.set(transition.id, new SvgTransition(this.svg, this.eventBus, transition));

        return transition.id;
    }

    connect(outId: string, inId: string): void {
        const arc = this.state.createArc(outId, inId);
        if (arc != null) {
    ...
    startConnect(outId: string): void {
        ...
        const outElement: PetriNetElement = this.state.getElement(outId);
        ...
        this.svg.on('mousemove', (event: MouseEvent) => {
            if (this.connectSvgLine == null) {
                this.connectSvgLine =
                    SvgArc.createLine(this.svg.select('.arcs'))
                        .attr('x1', x1)
                        .attr('y1', y1);
            }
    ...
    removeElement(id: string): void {
        ...
        svgElement.delete();
        this.state.getElementArcs(svgElement.model.id).forEach((arc) => this.removeArc(arc.id));
        this.state.removeElement(svgElement.model.id);
    }
    ...
    removeArc(id: string): void {
        ...
        this.state.removeArc(id);
    }

    updateDimensions(): void {
        const width = Math.max(
            ...this.state.getTransitions().map(transition => transition.x + Transition.WIDTH),
            ...this.state.getPlaces().map(place => place.cx + Place.RADIUS)
        );
        const height = Math.max(
            ...this.state.getTransitions().map(transition => transition.y + Transition.HEIGHT),
            ...this.state.getPlaces().map(place => place.cy + Place.RADIUS)
        );
    ...
    enableDragging(): void {
        this.eventBus.on(EventNames.ON_DRAG, (elementId) => {
            this.state.getElementArcs(elementId as string).forEach((arc) => {
    ...
    getNumberOfLayers(): number {
        return Math.max(0,
            ...this.state.getBlocks().map(block => block.getNumberOfLayers())
        );
    }

    getHierarchyDepth(): number {
        return Math.max(0,
            ...this.state.getBlocks().map(block => this.getHierarchyDepthRec(block))
        );
    }

    getBranchingFactor(): number {
        const blocks = this.getBlocks()
            .filter(block => !(block instanceof NodeBlock));

        return blocks
            .map(block => block.blocks.length)
            .reduce((sum, numberOfBlocks) => sum + numberOfBlocks, 0) / blocks.length;
    }
    ...
    private getBlocksRec(block: Block): Block[] {
        return [...block.blocks, ...block.blocks.flatMap(block1 => this.getBlocksRec(block1))];
    }
    ...
    private static updateInPosition(
        ...
        if (inElement instanceof Transition) {
            [newX2, newY2] =
                PetriNetSvgManager.calculateTransitionInPosition(arc, inElement);
