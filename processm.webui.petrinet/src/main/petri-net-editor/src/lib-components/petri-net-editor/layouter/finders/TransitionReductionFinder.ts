...
export class TransitionReductionFinder implements ReductionFinder {
    ...
    find(state: PetriNetState): [Reduction | null, number] {
        ...
        for (const candidateTransition of transitions) {
            const canBeReduced = this.evaluateTransitionForReduction(candidateTransition, state);
            if (!canBeReduced) {
                continue;
            }

            const solutionTransition = this.getSingleTransitionForBlock(state, candidateTransition, transitions);
            ...
            if (state.getPrePlaces(candidateTransition).length > 0 || state.getPostPlaces(candidateTransition).length > 0) {
                const reduction = new TransitionReduction(candidateTransition, solution, this._isReversed);
                const score = reduction.getScore(state);

                if (score === TransitionReduction.MAX_SCORE) {
    ...
    private evaluateTransitionForReduction(transition: Transition, state: PetriNetState): boolean {
        ...
        return postPlaces.some(place => state.getPreTransitions(place).length >= 2);
    }
    ...
    private getSingleTransitionForBlock(
        ...
        for (const transition of transitions) {
            ...
            const prePlacesDiff = state.getPrePlaces(transition)
                .filter(place => !this._isReversed ? !prePlaces.has(place) : !postPlaces.has(place));
            const postPlacesDiff = state.getPostPlaces(transition)
                .filter(place => !this._isReversed ? !postPlaces.has(place) : !prePlaces.has(place));
