...
export class ShortLoopReduction implements Reduction {
    ...
    apply(state: PetriNetState): Block {
        ...
        if (this.isPlaceReducible(state)) {
            block = new VerticalBlock(state.getBlock(this.placeCandidate.id), state.getBlock(this.transitionsCandidate.id), false);
            ...
            block = new VerticalBlock(state.getBlock(this.transitionsCandidate.id), state.getBlock(this.placeCandidate.id), false);
    ...
    getScore(state: PetriNetState): number {
        ...
        this._score = this.isPlaceReducible(state) || this.isTransitionReducible(state)
    ...
    private isPlaceReducible(state: PetriNetState): boolean {
        ...
        const preTransition = preTransitions
            .find(transition => transition == this.transitionsCandidate);

        const postTransition = postTransitions
            .find(transition => transition == this.transitionsCandidate);
    ...
    private isTransitionReducible(state: PetriNetState): boolean {
        ...
        const prePlace = prePlaces
            .find(place => place == this.placeCandidate);

        const postPlace = postPlaces
            .find(place => place == this.placeCandidate);
