...
import { HorizontalBlock } from '@/lib-components/petri-net-editor/layouter/blocks/HorizontalBlock';
...
export class AbstractionReduction implements Reduction {
    ...
    constructor(placeToReduce: Place, transitionToReduce: Transition, preTransitions: Transition[], postPlaces: Place[]) {
    ...
    apply(state: PetriNetState): Block {
        ...
        if (this._preTransitions.length == 1 && state.getPostPlaces(this._preTransitions[0]).length == 1) {
            bestTransition = this._preTransitions[0];
        } else if (this._postPlaces.length == 1 && state.getPreTransitions(this._postPlaces[0]).length == 1) {
    ...
    getScore(_: PetriNetState): number {
        ...
        let scoreDivisor: number = this._preTransitions.length + this._postPlaces.length;
        if (this._preTransitions.length != 0 && this._postPlaces.length != 0) {
            scoreDivisor /= 2;
        }
