...
import { Arc } from '@/lib-components/petri-net-editor/model/Arc';

export class PnmlSerializer {
    static serialize(state: PetriNetState, name: string): string {
        const parser = new DOMParser();
        const pnmlDoc: Document = parser.parseFromString(`<?xml version="1.0" encoding="UTF-8"?>
            <pnml>
        ...
        PnmlSerializer.appendTransitions(page, state.getTransitions());
        PnmlSerializer.appendPlaces(page, state.getPlaces());
        PnmlSerializer.appendArcs(page, state.getArcs());
    ...
    static deserialize(data: string): PetriNetState {
        ...
        places.forEach((place) => state.createPlace({
            x: place.x ?? 0,
            y: place.y ?? 0,
            text: place.text,
            tokenCount: 0,
        ...
        transitions.forEach((transition) => state.createTransition({
            x: transition.x ?? 0,
            y: transition.y ?? 0,
            text: transition.text,
            id: transition.id
        }));
        arcs.forEach((arc) => state.createArc(arc.outElementId, arc.inElementId, arc.id));
    ...
    private static deserializePlaces(pnmlDoc: Document): PlaceDto[] {
        ...
        for (const pnmlPlace of pnmlDoc.getElementsByTagName('place')) {
            ...
            const initialMarking = pnmlPlace.getElementsByTagName('initialMarking')[0]
    ...
    private static appendTransitions(page: Element, transitions: Transition[]): void {
